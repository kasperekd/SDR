...
bool init_adalm_pluto(const std::string& ip, const StreamConfig& txcfg,
    ...
    iio_attr_write_string(iio_channel_find_attr(tx_chn, "rf_port_select"), txcfg.rfport);
    iio_attr_write_longlong(iio_channel_find_attr(tx_chn, "rf_bandwidth"), txcfg.bw_hz);
    iio_attr_write_longlong(iio_channel_find_attr(tx_chn, "sampling_frequency"), txcfg.fs_hz);
    iio_attr_write_longlong(iio_channel_find_attr(iio_device_find_channel(devices.phy_dev, "altvoltage1", true), "frequency"), txcfg.lo_hz);
    iio_attr_write_longlong(iio_channel_find_attr(tx_chn, "hardwaregain"), 70);

    // RX
    struct iio_channel* rx_chn =
        iio_device_find_channel(devices.phy_dev, "voltage0", false);
    iio_attr_write_string(iio_channel_find_attr(rx_chn, "rf_port_select"), rxcfg.rfport);
    iio_attr_write_longlong(iio_channel_find_attr(rx_chn, "rf_bandwidth"), rxcfg.bw_hz);
    iio_attr_write_longlong(iio_channel_find_attr(rx_chn, "sampling_frequency"), rxcfg.fs_hz);
    iio_attr_write_longlong(iio_channel_find_attr(iio_device_find_channel(devices.phy_dev, "altvoltage0", true), "frequency"), rxcfg.lo_hz);
    ...
    // iio_attr_write_string(rx_gain_attr, "manual");
